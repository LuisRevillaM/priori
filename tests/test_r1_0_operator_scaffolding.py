...
R1_SHARED_CODE_FORBIDDEN_LITERALS = (
    "operator:window",
    "execute_window",
)
...
class R10OperatorScaffoldingTests(unittest.TestCase):
    def test_operator_registry_is_explicit_and_complete(self) -> None:
        self.assertEqual(
            ("project_onto_axis", "delta_across_anchor", "extremum_over_set", "window"),
            tuple(signature.name for signature in OPERATOR_SIGNATURES),
        )
        self.assertEqual(
            {
                ("project_onto_axis", "0.1.0"),
                ("delta_across_anchor", "0.1.0"),
    ...
    def test_r1_operator_names_do_not_leak_into_shared_runtime_code(self) -> None:
        for module in (binder_module, executor_module):
            ...
            for name in R1_SHARED_CODE_RATCHET_NAMES:
                pattern = re.compile(rf"(?<![A-Za-z0-9_]){re.escape(name)}(?![A-Za-z0-9_])")
                self.assertIsNone(pattern.search(source), f"{name} leaked into {module.__name__}")
            for literal in R1_SHARED_CODE_FORBIDDEN_LITERALS:
                self.assertNotIn(literal, source, f"{literal} leaked into {module.__name__}")
