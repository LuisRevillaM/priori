...
class Deploy2LazyHydrationTests(unittest.TestCase):
    ...
    def test_startup_merges_fragments_without_opening_execution_cache_payloads(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            cache = root / "cache"
            ...
            with (
                ...
                prewarm_film_room_flagships_from_certified_tables()
                prewarm_film_room_flagships(output_root=root / "runtime")
        ...
        fragile_record = next(
            ...
            and record.get("prewarm_kind") == "descriptor_index_upgrade"
        )
