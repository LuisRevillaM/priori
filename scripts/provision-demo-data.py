...
def main() -> int:
    ...
    if dataset_satisfies_manifest(args.dataset_root, manifest):
    ...
    with tempfile.TemporaryDirectory(prefix="priori-demo-data-") as temp_dir:
        ...
        if not dataset_satisfies_manifest(staged, manifest):
            print(json.dumps(missing_report(staged, manifest), indent=2, sort_keys=True))
...
def dataset_satisfies_manifest(dataset_root: Path, manifest: dict[str, Any]) -> bool:
    ...
    return all((dataset_root / "canonical" / "v1" / path).exists() for path in required) and all(
        (dataset_root / "raw" / "idsse" / "figshare-28196177-v1" / path).exists()
        for path in raw_required
    )


def missing_report(dataset_root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    required = [str(path) for path in manifest.get("required_paths") or []]
    raw_required = [str(path) for path in manifest.get("required_raw_paths") or []]
    return {
