...
export TQE_NODE_CACHE_ROOT="${TQE_NODE_CACHE_ROOT:-$TQE_CACHE_ROOT/node-output}"
...
mkdir -p "$TQE_DATA_ROOT" "$TQE_RAW_ROOT" "$TQE_RUNTIME_ROOT" "$TQE_CACHE_ROOT" "$TQE_NODE_CACHE_ROOT"

provision_demo_data() {
  python /app/scripts/provision-demo-data.py \
    --dataset-root /var/data/dataset \
    --cache-root "$TQE_CACHE_ROOT" \
    --runtime-root "$TQE_RUNTIME_ROOT" \
