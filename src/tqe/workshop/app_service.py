...
def load_film_room_descriptor_index(*, output_root: Path) -> None:
    """Load chain descriptors from cache metadata without executing a plan."""
    ...
    with FILM_ROOM_PREWARM_LOCK:
        ...
        FILM_ROOM_PREWARM_STATE["descriptor_index_started_at"] = utc_iso_seconds()
    ...
    for spec in film_room_flagship_specs():
        ...
        if not plan_path.exists():
            print(f"Film Room descriptor load skipped missing plan: {plan_path}", flush=True)
            with FILM_ROOM_PREWARM_LOCK:
                for item in FILM_ROOM_PREWARM_STATE["items"]:
                    if item.get("key") == str(spec["key"]):
            ...
            continue
        started_at = time.monotonic()
        print(f"Loading Film Room descriptor metadata for {plan_path}...", flush=True)
        with FILM_ROOM_PREWARM_LOCK:
            for item in FILM_ROOM_PREWARM_STATE["items"]:
                if item.get("key") == str(spec["key"]):
...
def prewarm_film_room_flagships(*, output_root: Path) -> None:
    """Compatibility wrapper for the metadata-only descriptor loader."""

    load_film_room_descriptor_index(output_root=output_root)


def load_film_room_descriptor_index_safely(*, output_root: Path) -> None:
    try:
        load_film_room_descriptor_index(output_root=output_root)
        print("Film Room descriptor index load complete.", flush=True)
    except Exception as exc:  # noqa: BLE001 - metadata failure must preserve the table gallery.
        print(
            json.dumps(
                {
                    "event": "film_room_descriptor_index_load_failed",
...
def prewarm_film_room_flagships_safely(*, output_root: Path) -> None:
    """Compatibility wrapper for the fail-safe metadata loader."""

    load_film_room_descriptor_index_safely(output_root=output_root)
...
def start_film_room_prewarm_thread(*, output_root: Path) -> threading.Thread:
    ...
    thread = threading.Thread(
        target=load_film_room_descriptor_index_safely,
        kwargs={"output_root": output_root},
        name="film-room-descriptor-index",
        daemon=True,
    )
    thread.start()
    return thread
...
def initialize_film_room_prewarm(
    *,
    ...
    load_film_room_descriptor_index_safely(output_root=output_root)
    requested_execution_prewarm = (
        WORKBENCH_PREWARM_FILM_ROOM if execution_enabled is None else execution_enabled
    )
    execution_status = "requested_but_retired" if requested_execution_prewarm else "disabled"
    print(
        "Film Room execution prewarm "
        f"{execution_status}; disk descriptor index is loaded independently.",
        flush=True,
    )
    return None
