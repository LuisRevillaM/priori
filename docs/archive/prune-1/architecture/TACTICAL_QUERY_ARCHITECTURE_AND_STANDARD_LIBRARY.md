...
Original path: `docs/TACTICAL_QUERY_ARCHITECTURE_AND_STANDARD_LIBRARY.md`

Quarantined under PRUNE-1 after inventory commit `7a07dde`.

Reason: superseded architecture brief that presents legacy predicate/operator
grammar as current composition guidance. The legacy `operators` key remains
live machinery in checked-in plans; this document is archived so it no longer
acts as current grammar documentation. Use ADRs, semantic registry artifacts,
current runtime code, and packet reports for current architecture.
