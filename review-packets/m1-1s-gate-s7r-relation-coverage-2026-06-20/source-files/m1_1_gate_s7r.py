...
def build_report() -> dict[str, Any]:
    ...
    report = {
        "schema_version": "1.0",
        "milestone": "M1.1S",
        "gate": "Gate_S7R_relation_coverage_witness_semantics",
...
def validate_agent_safety_limits() -> list[dict[str, Any]]:
    ...
    valid = runtime_failed and static_failed
    return [
        pass_check(
            "s7r.agent_safety_limits_enforced",
            "relation expansion and declared execution-cost limits fail visibly",
            {"runtime_message": runtime_message, "static_failed": static_failed},
        )
        if valid
        else fail_check(
            "s7r.agent_safety_limits_enforced",
            "agent safety limits are not enforced visibly",
            {"runtime_message": runtime_message, "runtime_failed": runtime_failed, "static_failed": static_failed},
        )
    ]
