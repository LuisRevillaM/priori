/* Generated by scripts/workbench_alpha/generate_contracts.py. */
import type { FromSchema } from "json-schema-to-ts";

export const apiSchemas = {
  "BootstrapResponse": {
    "$defs": {
      "CapabilitySummaryResponse": {
        "additionalProperties": false,
        "properties": {
          "execute_tool_description": {
            "additionalProperties": true,
            "title": "Execute Tool Description",
            "type": "object"
          },
          "operator_count": {
            "title": "Operator Count",
            "type": "integer"
          },
          "primitive_count": {
            "title": "Primitive Count",
            "type": "integer"
          },
          "relation_count": {
            "title": "Relation Count",
            "type": "integer"
          },
          "tools": {
            "items": {
              "type": "string"
            },
            "title": "Tools",
            "type": "array"
          }
        },
        "required": [
          "primitive_count",
          "relation_count",
          "operator_count",
          "tools",
          "execute_tool_description"
        ],
        "title": "CapabilitySummaryResponse",
        "type": "object"
      },
      "ModelStatusResponse": {
        "additionalProperties": false,
        "properties": {
          "available": {
            "title": "Available",
            "type": "boolean"
          },
          "message": {
            "title": "Message",
            "type": "string"
          },
          "status": {
            "title": "Status",
            "type": "string"
          }
        },
        "required": [
          "available",
          "status",
          "message"
        ],
        "title": "ModelStatusResponse",
        "type": "object"
      },
      "PresetResponse": {
        "additionalProperties": false,
        "properties": {
          "label": {
            "title": "Label",
            "type": "string"
          },
          "plan_hash": {
            "title": "Plan Hash",
            "type": "string"
          },
          "preset_id": {
            "enum": [
              "approved_block_shift",
              "experimental_corridor",
              "experimental_high_bypass",
              "experimental_line_break_support_response"
            ],
            "title": "Preset Id",
            "type": "string"
          },
          "recipe": {
            "$ref": "#/$defs/RecipeCardResponse"
          }
        },
        "required": [
          "preset_id",
          "label",
          "recipe",
          "plan_hash"
        ],
        "title": "PresetResponse",
        "type": "object"
      },
      "RecipeCardResponse": {
        "additionalProperties": false,
        "properties": {
          "allowed_claims": {
            "items": {
              "type": "string"
            },
            "title": "Allowed Claims",
            "type": "array"
          },
          "description": {
            "title": "Description",
            "type": "string"
          },
          "disallowed_claims": {
            "items": {
              "type": "string"
            },
            "title": "Disallowed Claims",
            "type": "array"
          },
          "display_name": {
            "title": "Display Name",
            "type": "string"
          },
          "limitations": {
            "items": {
              "type": "string"
            },
            "title": "Limitations",
            "type": "array"
          },
          "output_classifications": {
            "items": {
              "type": "string"
            },
            "title": "Output Classifications",
            "type": "array"
          },
          "recipe_id": {
            "title": "Recipe Id",
            "type": "string"
          },
          "recipe_version": {
            "title": "Recipe Version",
            "type": "string"
          },
          "state": {
            "enum": [
              "APPROVED",
              "EXPERIMENTAL",
              "USER_SAVED",
              "DEPRECATED"
            ],
            "title": "State",
            "type": "string"
          }
        },
        "required": [
          "recipe_id",
          "recipe_version",
          "state",
          "display_name",
          "description",
          "allowed_claims",
          "disallowed_claims",
          "limitations",
          "output_classifications"
        ],
        "title": "RecipeCardResponse",
        "type": "object"
      },
      "ServiceStatusResponse": {
        "additionalProperties": false,
        "properties": {
          "mcp_adapter": {
            "title": "Mcp Adapter",
            "type": "boolean"
          },
          "name": {
            "title": "Name",
            "type": "string"
          }
        },
        "required": [
          "name",
          "mcp_adapter"
        ],
        "title": "ServiceStatusResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "capabilities": {
        "$ref": "#/$defs/CapabilitySummaryResponse"
      },
      "model": {
        "$ref": "#/$defs/ModelStatusResponse"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "presets": {
        "items": {
          "$ref": "#/$defs/PresetResponse"
        },
        "title": "Presets",
        "type": "array"
      },
      "service": {
        "$ref": "#/$defs/ServiceStatusResponse"
      }
    },
    "required": [
      "ok",
      "service",
      "model",
      "presets",
      "capabilities"
    ],
    "title": "BootstrapResponse",
    "type": "object"
  },
  "CoachInterpretResponse": {
    "$defs": {
      "CoachMomentResponse": {
        "additionalProperties": false,
        "properties": {
          "evidence_fields": {
            "items": {
              "type": "string"
            },
            "title": "Evidence Fields",
            "type": "array"
          },
          "moment_id": {
            "title": "Moment Id",
            "type": "string"
          },
          "replay_payload": {
            "anyOf": [
              {
                "additionalProperties": true,
                "type": "object"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Replay Payload"
          },
          "result_count": {
            "title": "Result Count",
            "type": "integer"
          }
        },
        "required": [
          "moment_id",
          "result_count",
          "evidence_fields"
        ],
        "title": "CoachMomentResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "audit": {
        "additionalProperties": true,
        "title": "Audit",
        "type": "object"
      },
      "contract_hash": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Contract Hash"
      },
      "display_answer": {
        "title": "Display Answer",
        "type": "string"
      },
      "gap": {
        "anyOf": [
          {
            "additionalProperties": true,
            "type": "object"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Gap"
      },
      "interpretation_rules": {
        "items": {
          "type": "string"
        },
        "title": "Interpretation Rules",
        "type": "array"
      },
      "match_scope": {
        "additionalProperties": true,
        "title": "Match Scope",
        "type": "object"
      },
      "meaning_definition": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Meaning Definition"
      },
      "moments": {
        "items": {
          "$ref": "#/$defs/CoachMomentResponse"
        },
        "title": "Moments",
        "type": "array"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "plan_hash": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Plan Hash"
      },
      "query": {
        "title": "Query",
        "type": "string"
      },
      "result_count": {
        "default": 0,
        "title": "Result Count",
        "type": "integer"
      },
      "status": {
        "enum": [
          "moment_found",
          "no_moments_found",
          "clarification_required",
          "understood_but_not_expressible"
        ],
        "title": "Status",
        "type": "string"
      },
      "suggestions": {
        "items": {
          "type": "string"
        },
        "title": "Suggestions",
        "type": "array"
      }
    },
    "required": [
      "ok",
      "status",
      "query",
      "display_answer"
    ],
    "title": "CoachInterpretResponse",
    "type": "object"
  },
  "ConfirmationResponse": {
    "additionalProperties": false,
    "properties": {
      "confirmation": {
        "additionalProperties": true,
        "title": "Confirmation",
        "type": "object"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      }
    },
    "required": [
      "ok",
      "confirmation"
    ],
    "title": "ConfirmationResponseEnvelope",
    "type": "object"
  },
  "ErrorResponse": {
    "additionalProperties": false,
    "properties": {
      "details": {
        "additionalProperties": true,
        "title": "Details",
        "type": "object"
      },
      "error_code": {
        "title": "Error Code",
        "type": "string"
      },
      "message": {
        "title": "Message",
        "type": "string"
      },
      "ok": {
        "const": false,
        "title": "Ok",
        "type": "boolean"
      }
    },
    "required": [
      "ok",
      "error_code",
      "message"
    ],
    "title": "ErrorResponse",
    "type": "object"
  },
  "ExecutionProgressResponse": {
    "additionalProperties": false,
    "properties": {
      "cache_key": {
        "title": "Cache Key",
        "type": "string"
      },
      "cache_status": {
        "enum": [
          "HIT",
          "MISS"
        ],
        "title": "Cache Status",
        "type": "string"
      },
      "message": {
        "title": "Message",
        "type": "string"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "stages": {
        "items": {
          "type": "string"
        },
        "title": "Stages",
        "type": "array"
      }
    },
    "required": [
      "ok",
      "cache_key",
      "cache_status",
      "message",
      "stages"
    ],
    "title": "ExecutionProgressResponse",
    "type": "object"
  },
  "ExecutionResponse": {
    "$defs": {
      "ExecutionProgressResponse": {
        "additionalProperties": false,
        "properties": {
          "cache_key": {
            "title": "Cache Key",
            "type": "string"
          },
          "cache_status": {
            "enum": [
              "HIT",
              "MISS"
            ],
            "title": "Cache Status",
            "type": "string"
          },
          "message": {
            "title": "Message",
            "type": "string"
          },
          "ok": {
            "const": true,
            "title": "Ok",
            "type": "boolean"
          },
          "stages": {
            "items": {
              "type": "string"
            },
            "title": "Stages",
            "type": "array"
          }
        },
        "required": [
          "ok",
          "cache_key",
          "cache_status",
          "message",
          "stages"
        ],
        "title": "ExecutionProgressResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "cache": {
        "$ref": "#/$defs/ExecutionProgressResponse"
      },
      "execution": {
        "additionalProperties": true,
        "title": "Execution",
        "type": "object"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      }
    },
    "required": [
      "ok",
      "execution",
      "cache"
    ],
    "title": "ExecutionResponseEnvelope",
    "type": "object"
  },
  "FilmRoomAskResponse": {
    "$defs": {
      "FilmRoomAnswerResponse": {
        "additionalProperties": false,
        "properties": {
          "certified_evidence_rows": {
            "items": {
              "additionalProperties": true,
              "type": "object"
            },
            "title": "Certified Evidence Rows",
            "type": "array"
          },
          "compiled_chips": {
            "items": {
              "type": "string"
            },
            "title": "Compiled Chips",
            "type": "array"
          },
          "document": {
            "additionalProperties": true,
            "title": "Document",
            "type": "object"
          },
          "evidence_rows_kind": {
            "enum": [
              "certified",
              "runtime"
            ],
            "title": "Evidence Rows Kind",
            "type": "string"
          },
          "executions": {
            "items": {
              "$ref": "#/$defs/FilmRoomExecutionRecordResponse"
            },
            "title": "Executions",
            "type": "array"
          },
          "interval_metric": {
            "anyOf": [
              {
                "$ref": "#/$defs/FilmRoomIntervalMetricResponse"
              },
              {
                "type": "null"
              }
            ],
            "default": null
          },
          "meaning_expression": {
            "anyOf": [
              {
                "additionalProperties": true,
                "type": "object"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Meaning Expression"
          },
          "moment_total_count": {
            "title": "Moment Total Count",
            "type": "integer"
          },
          "moments": {
            "items": {
              "$ref": "#/$defs/FilmRoomMomentResponse"
            },
            "title": "Moments",
            "type": "array"
          },
          "provenance": {
            "$ref": "#/$defs/FilmRoomProvenanceResponse"
          },
          "raw_evidence": {
            "additionalProperties": true,
            "title": "Raw Evidence",
            "type": "object"
          },
          "replay": {
            "anyOf": [
              {
                "$ref": "#/$defs/ReplayPayloadResponse"
              },
              {
                "type": "null"
              }
            ],
            "default": null
          },
          "runtime_evidence_rows": {
            "items": {
              "additionalProperties": true,
              "type": "object"
            },
            "title": "Runtime Evidence Rows",
            "type": "array"
          },
          "status": {
            "const": "answer_ready",
            "title": "Status",
            "type": "string"
          },
          "visible_moment_count": {
            "title": "Visible Moment Count",
            "type": "integer"
          }
        },
        "required": [
          "status",
          "compiled_chips",
          "document",
          "certified_evidence_rows",
          "runtime_evidence_rows",
          "evidence_rows_kind",
          "moments",
          "moment_total_count",
          "visible_moment_count",
          "executions",
          "provenance"
        ],
        "title": "FilmRoomAnswerResponse",
        "type": "object"
      },
      "FilmRoomExecutionRecordResponse": {
        "additionalProperties": false,
        "properties": {
          "bound_record": {
            "additionalProperties": true,
            "title": "Bound Record",
            "type": "object"
          },
          "cache_after_execute": {
            "additionalProperties": true,
            "title": "Cache After Execute",
            "type": "object"
          },
          "cache_before": {
            "additionalProperties": true,
            "title": "Cache Before",
            "type": "object"
          },
          "confirmation": {
            "additionalProperties": true,
            "title": "Confirmation",
            "type": "object"
          },
          "draft_record": {
            "additionalProperties": true,
            "title": "Draft Record",
            "type": "object"
          },
          "execution": {
            "additionalProperties": true,
            "title": "Execution",
            "type": "object"
          },
          "role": {
            "title": "Role",
            "type": "string"
          },
          "runtime_evidence_sources": {
            "items": {
              "additionalProperties": true,
              "type": "object"
            },
            "title": "Runtime Evidence Sources",
            "type": "array"
          },
          "submit": {
            "additionalProperties": true,
            "title": "Submit",
            "type": "object"
          },
          "validation": {
            "additionalProperties": true,
            "title": "Validation",
            "type": "object"
          }
        },
        "required": [
          "role",
          "submit",
          "validation",
          "confirmation",
          "cache_before",
          "execution",
          "cache_after_execute",
          "draft_record",
          "bound_record"
        ],
        "title": "FilmRoomExecutionRecordResponse",
        "type": "object"
      },
      "FilmRoomIntervalMetricResponse": {
        "additionalProperties": false,
        "properties": {
          "label": {
            "title": "Label",
            "type": "string"
          },
          "lower": {
            "title": "Lower",
            "type": "number"
          },
          "observed": {
            "title": "Observed",
            "type": "number"
          },
          "source": {
            "additionalProperties": true,
            "title": "Source",
            "type": "object"
          },
          "unknown_count": {
            "title": "Unknown Count",
            "type": "integer"
          },
          "upper": {
            "title": "Upper",
            "type": "number"
          }
        },
        "required": [
          "label",
          "observed",
          "lower",
          "upper",
          "unknown_count",
          "source"
        ],
        "title": "FilmRoomIntervalMetricResponse",
        "type": "object"
      },
      "FilmRoomMomentResponse": {
        "additionalProperties": false,
        "properties": {
          "anchor_frame_id": {
            "title": "Anchor Frame Id",
            "type": "integer"
          },
          "chain_reason": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Chain Reason"
          },
          "chain_status": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Chain Status"
          },
          "classification": {
            "title": "Classification",
            "type": "string"
          },
          "end_frame_id": {
            "anyOf": [
              {
                "type": "integer"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "End Frame Id"
          },
          "evidence_overlay": {
            "additionalProperties": true,
            "title": "Evidence Overlay",
            "type": "object"
          },
          "evidence_row": {
            "anyOf": [
              {
                "additionalProperties": true,
                "type": "object"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Evidence Row"
          },
          "match_id": {
            "title": "Match Id",
            "type": "string"
          },
          "match_time_ms": {
            "anyOf": [
              {
                "type": "integer"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Match Time Ms"
          },
          "period": {
            "title": "Period",
            "type": "string"
          },
          "replay_end_frame_id": {
            "anyOf": [
              {
                "type": "integer"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Replay End Frame Id"
          },
          "replay_start_frame_id": {
            "anyOf": [
              {
                "type": "integer"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Replay Start Frame Id"
          },
          "replay_window_id": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Replay Window Id"
          },
          "requested_evidence": {
            "additionalProperties": true,
            "title": "Requested Evidence",
            "type": "object"
          },
          "result_id": {
            "title": "Result Id",
            "type": "string"
          },
          "source_kind": {
            "enum": [
              "result",
              "target",
              "chain_record",
              "certified_table_partition"
            ],
            "title": "Source Kind",
            "type": "string"
          },
          "start_frame_id": {
            "anyOf": [
              {
                "type": "integer"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Start Frame Id"
          },
          "unknown_reason": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Unknown Reason"
          }
        },
        "required": [
          "result_id",
          "source_kind",
          "classification",
          "match_id",
          "period",
          "anchor_frame_id",
          "requested_evidence"
        ],
        "title": "FilmRoomMomentResponse",
        "type": "object"
      },
      "FilmRoomProvenanceResponse": {
        "additionalProperties": false,
        "properties": {
          "bound_plan_hashes": {
            "additionalProperties": {
              "type": "string"
            },
            "title": "Bound Plan Hashes",
            "type": "object"
          },
          "canonical_sources": {
            "additionalProperties": {
              "type": "string"
            },
            "title": "Canonical Sources",
            "type": "object"
          },
          "certified_period_records_hash": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Certified Period Records Hash"
          },
          "certified_plan_path": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Certified Plan Path"
          },
          "certified_table_hash": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Certified Table Hash"
          },
          "certified_table_path": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Certified Table Path"
          },
          "expression_hash": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Expression Hash"
          },
          "plan_hash": {
            "title": "Plan Hash",
            "type": "string"
          },
          "replay_window_id": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Replay Window Id"
          },
          "runtime_commit": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Runtime Commit"
          },
          "synthesized_document_hash": {
            "title": "Synthesized Document Hash",
            "type": "string"
          },
          "tree": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Tree"
          }
        },
        "required": [
          "plan_hash",
          "synthesized_document_hash",
          "bound_plan_hashes",
          "canonical_sources"
        ],
        "title": "FilmRoomProvenanceResponse",
        "type": "object"
      },
      "PitchResponse": {
        "additionalProperties": false,
        "properties": {
          "coordinate_contract": {
            "title": "Coordinate Contract",
            "type": "string"
          },
          "length_m": {
            "title": "Length M",
            "type": "number"
          },
          "width_m": {
            "title": "Width M",
            "type": "number"
          }
        },
        "required": [
          "length_m",
          "width_m",
          "coordinate_contract"
        ],
        "title": "PitchResponse",
        "type": "object"
      },
      "ReplayEntityResponse": {
        "additionalProperties": false,
        "properties": {
          "entity_id": {
            "title": "Entity Id",
            "type": "string"
          },
          "entity_type": {
            "title": "Entity Type",
            "type": "string"
          },
          "team_id": {
            "title": "Team Id",
            "type": "string"
          },
          "team_role": {
            "title": "Team Role",
            "type": "string"
          },
          "x_m": {
            "title": "X M",
            "type": "number"
          },
          "y_m": {
            "title": "Y M",
            "type": "number"
          }
        },
        "required": [
          "team_id",
          "team_role",
          "entity_id",
          "entity_type",
          "x_m",
          "y_m"
        ],
        "title": "ReplayEntityResponse",
        "type": "object"
      },
      "ReplayFrameResponse": {
        "additionalProperties": false,
        "properties": {
          "entities": {
            "items": {
              "$ref": "#/$defs/ReplayEntityResponse"
            },
            "title": "Entities",
            "type": "array"
          },
          "frame_id": {
            "title": "Frame Id",
            "type": "integer"
          },
          "timestamp_utc": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Timestamp Utc"
          }
        },
        "required": [
          "frame_id",
          "entities"
        ],
        "title": "ReplayFrameResponse",
        "type": "object"
      },
      "ReplayPayloadResponse": {
        "additionalProperties": false,
        "properties": {
          "anchor_frame_id": {
            "title": "Anchor Frame Id",
            "type": "integer"
          },
          "canonical_sources": {
            "additionalProperties": {
              "type": "string"
            },
            "title": "Canonical Sources",
            "type": "object"
          },
          "end_frame_id": {
            "title": "End Frame Id",
            "type": "integer"
          },
          "frame_rate_hz": {
            "title": "Frame Rate Hz",
            "type": "number"
          },
          "frames": {
            "items": {
              "$ref": "#/$defs/ReplayFrameResponse"
            },
            "title": "Frames",
            "type": "array"
          },
          "generated_at": {
            "title": "Generated At",
            "type": "string"
          },
          "match_id": {
            "title": "Match Id",
            "type": "string"
          },
          "overlays": {
            "additionalProperties": true,
            "title": "Overlays",
            "type": "object"
          },
          "period": {
            "title": "Period",
            "type": "string"
          },
          "pitch": {
            "$ref": "#/$defs/PitchResponse"
          },
          "replay_window_id": {
            "title": "Replay Window Id",
            "type": "string"
          },
          "schema_version": {
            "title": "Schema Version",
            "type": "string"
          },
          "source_id": {
            "title": "Source Id",
            "type": "string"
          },
          "source_kind": {
            "enum": [
              "result",
              "target",
              "chain_record",
              "certified_table_partition"
            ],
            "title": "Source Kind",
            "type": "string"
          },
          "start_frame_id": {
            "title": "Start Frame Id",
            "type": "integer"
          }
        },
        "required": [
          "schema_version",
          "replay_window_id",
          "source_kind",
          "source_id",
          "match_id",
          "period",
          "frame_rate_hz",
          "start_frame_id",
          "end_frame_id",
          "anchor_frame_id",
          "generated_at",
          "canonical_sources",
          "pitch",
          "frames"
        ],
        "title": "ReplayPayloadResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "answer": {
        "anyOf": [
          {
            "$ref": "#/$defs/FilmRoomAnswerResponse"
          },
          {
            "type": "null"
          }
        ],
        "default": null
      },
      "clarification": {
        "anyOf": [
          {
            "additionalProperties": true,
            "type": "object"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Clarification"
      },
      "hermes": {
        "additionalProperties": true,
        "title": "Hermes",
        "type": "object"
      },
      "latency_breakdown_ms": {
        "additionalProperties": {
          "type": "integer"
        },
        "title": "Latency Breakdown Ms",
        "type": "object"
      },
      "latency_ms": {
        "title": "Latency Ms",
        "type": "integer"
      },
      "model": {
        "title": "Model",
        "type": "string"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "outcome": {
        "enum": [
          "expression",
          "clarification_required",
          "understood_but_not_expressible",
          "unsupported_modality"
        ],
        "title": "Outcome",
        "type": "string"
      },
      "provider": {
        "title": "Provider",
        "type": "string"
      },
      "refusal": {
        "anyOf": [
          {
            "additionalProperties": true,
            "type": "object"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Refusal"
      },
      "request_text": {
        "title": "Request Text",
        "type": "string"
      }
    },
    "required": [
      "ok",
      "outcome",
      "request_text",
      "provider",
      "model",
      "latency_ms",
      "latency_breakdown_ms",
      "hermes"
    ],
    "title": "FilmRoomAskResponse",
    "type": "object"
  },
  "FilmRoomBootstrapResponse": {
    "additionalProperties": false,
    "properties": {
      "answer": {
        "anyOf": [
          {
            "additionalProperties": true,
            "type": "object"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Answer"
      },
      "billing_surface": {
        "title": "Billing Surface",
        "type": "string"
      },
      "flagship_plan_hashes": {
        "additionalProperties": {
          "anyOf": [
            {
              "type": "string"
            },
            {
              "type": "null"
            }
          ]
        },
        "title": "Flagship Plan Hashes",
        "type": "object"
      },
      "flagship_responses": {
        "additionalProperties": {
          "additionalProperties": true,
          "type": "object"
        },
        "title": "Flagship Responses",
        "type": "object"
      },
      "model": {
        "title": "Model",
        "type": "string"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "prewarm_records": {
        "items": {
          "additionalProperties": true,
          "type": "object"
        },
        "title": "Prewarm Records",
        "type": "array"
      },
      "prewarmed_response": {
        "anyOf": [
          {
            "additionalProperties": true,
            "type": "object"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Prewarmed Response"
      },
      "provenance": {
        "anyOf": [
          {
            "additionalProperties": true,
            "type": "object"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Provenance"
      },
      "provider": {
        "title": "Provider",
        "type": "string"
      },
      "state": {
        "enum": [
          "ready",
          "warming"
        ],
        "title": "State",
        "type": "string"
      },
      "warming": {
        "anyOf": [
          {
            "additionalProperties": true,
            "type": "object"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Warming"
      }
    },
    "required": [
      "ok",
      "state",
      "provider",
      "model",
      "billing_surface",
      "flagship_plan_hashes",
      "prewarm_records"
    ],
    "title": "FilmRoomBootstrapResponse",
    "type": "object"
  },
  "FilmRoomReplayFrameResponse": {
    "$defs": {
      "ReplayEntityResponse": {
        "additionalProperties": false,
        "properties": {
          "entity_id": {
            "title": "Entity Id",
            "type": "string"
          },
          "entity_type": {
            "title": "Entity Type",
            "type": "string"
          },
          "team_id": {
            "title": "Team Id",
            "type": "string"
          },
          "team_role": {
            "title": "Team Role",
            "type": "string"
          },
          "x_m": {
            "title": "X M",
            "type": "number"
          },
          "y_m": {
            "title": "Y M",
            "type": "number"
          }
        },
        "required": [
          "team_id",
          "team_role",
          "entity_id",
          "entity_type",
          "x_m",
          "y_m"
        ],
        "title": "ReplayEntityResponse",
        "type": "object"
      },
      "ReplayFrameResponse": {
        "additionalProperties": false,
        "properties": {
          "entities": {
            "items": {
              "$ref": "#/$defs/ReplayEntityResponse"
            },
            "title": "Entities",
            "type": "array"
          },
          "frame_id": {
            "title": "Frame Id",
            "type": "integer"
          },
          "timestamp_utc": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Timestamp Utc"
          }
        },
        "required": [
          "frame_id",
          "entities"
        ],
        "title": "ReplayFrameResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "canonical_sources": {
        "additionalProperties": {
          "type": "string"
        },
        "title": "Canonical Sources",
        "type": "object"
      },
      "frame": {
        "$ref": "#/$defs/ReplayFrameResponse"
      },
      "frame_id": {
        "title": "Frame Id",
        "type": "integer"
      },
      "frame_sha256": {
        "title": "Frame Sha256",
        "type": "string"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "replay_window_id": {
        "title": "Replay Window Id",
        "type": "string"
      }
    },
    "required": [
      "ok",
      "replay_window_id",
      "frame_id",
      "frame_sha256",
      "canonical_sources",
      "frame"
    ],
    "title": "FilmRoomReplayFrameResponse",
    "type": "object"
  },
  "FilmRoomReplayWindowResponse": {
    "$defs": {
      "PitchResponse": {
        "additionalProperties": false,
        "properties": {
          "coordinate_contract": {
            "title": "Coordinate Contract",
            "type": "string"
          },
          "length_m": {
            "title": "Length M",
            "type": "number"
          },
          "width_m": {
            "title": "Width M",
            "type": "number"
          }
        },
        "required": [
          "length_m",
          "width_m",
          "coordinate_contract"
        ],
        "title": "PitchResponse",
        "type": "object"
      },
      "ReplayEntityResponse": {
        "additionalProperties": false,
        "properties": {
          "entity_id": {
            "title": "Entity Id",
            "type": "string"
          },
          "entity_type": {
            "title": "Entity Type",
            "type": "string"
          },
          "team_id": {
            "title": "Team Id",
            "type": "string"
          },
          "team_role": {
            "title": "Team Role",
            "type": "string"
          },
          "x_m": {
            "title": "X M",
            "type": "number"
          },
          "y_m": {
            "title": "Y M",
            "type": "number"
          }
        },
        "required": [
          "team_id",
          "team_role",
          "entity_id",
          "entity_type",
          "x_m",
          "y_m"
        ],
        "title": "ReplayEntityResponse",
        "type": "object"
      },
      "ReplayFrameResponse": {
        "additionalProperties": false,
        "properties": {
          "entities": {
            "items": {
              "$ref": "#/$defs/ReplayEntityResponse"
            },
            "title": "Entities",
            "type": "array"
          },
          "frame_id": {
            "title": "Frame Id",
            "type": "integer"
          },
          "timestamp_utc": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Timestamp Utc"
          }
        },
        "required": [
          "frame_id",
          "entities"
        ],
        "title": "ReplayFrameResponse",
        "type": "object"
      },
      "ReplayPayloadResponse": {
        "additionalProperties": false,
        "properties": {
          "anchor_frame_id": {
            "title": "Anchor Frame Id",
            "type": "integer"
          },
          "canonical_sources": {
            "additionalProperties": {
              "type": "string"
            },
            "title": "Canonical Sources",
            "type": "object"
          },
          "end_frame_id": {
            "title": "End Frame Id",
            "type": "integer"
          },
          "frame_rate_hz": {
            "title": "Frame Rate Hz",
            "type": "number"
          },
          "frames": {
            "items": {
              "$ref": "#/$defs/ReplayFrameResponse"
            },
            "title": "Frames",
            "type": "array"
          },
          "generated_at": {
            "title": "Generated At",
            "type": "string"
          },
          "match_id": {
            "title": "Match Id",
            "type": "string"
          },
          "overlays": {
            "additionalProperties": true,
            "title": "Overlays",
            "type": "object"
          },
          "period": {
            "title": "Period",
            "type": "string"
          },
          "pitch": {
            "$ref": "#/$defs/PitchResponse"
          },
          "replay_window_id": {
            "title": "Replay Window Id",
            "type": "string"
          },
          "schema_version": {
            "title": "Schema Version",
            "type": "string"
          },
          "source_id": {
            "title": "Source Id",
            "type": "string"
          },
          "source_kind": {
            "enum": [
              "result",
              "target",
              "chain_record",
              "certified_table_partition"
            ],
            "title": "Source Kind",
            "type": "string"
          },
          "start_frame_id": {
            "title": "Start Frame Id",
            "type": "integer"
          }
        },
        "required": [
          "schema_version",
          "replay_window_id",
          "source_kind",
          "source_id",
          "match_id",
          "period",
          "frame_rate_hz",
          "start_frame_id",
          "end_frame_id",
          "anchor_frame_id",
          "generated_at",
          "canonical_sources",
          "pitch",
          "frames"
        ],
        "title": "ReplayPayloadResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "replay": {
        "$ref": "#/$defs/ReplayPayloadResponse"
      },
      "replay_window_id": {
        "title": "Replay Window Id",
        "type": "string"
      }
    },
    "required": [
      "ok",
      "replay_window_id",
      "replay"
    ],
    "title": "FilmRoomReplayWindowResponse",
    "type": "object"
  },
  "HealthResponse": {
    "additionalProperties": false,
    "properties": {
      "mcp_adapter": {
        "title": "Mcp Adapter",
        "type": "boolean"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "service": {
        "title": "Service",
        "type": "string"
      }
    },
    "required": [
      "ok",
      "service",
      "mcp_adapter"
    ],
    "title": "HealthResponse",
    "type": "object"
  },
  "InspectResultResponse": {
    "$defs": {
      "PitchResponse": {
        "additionalProperties": false,
        "properties": {
          "coordinate_contract": {
            "title": "Coordinate Contract",
            "type": "string"
          },
          "length_m": {
            "title": "Length M",
            "type": "number"
          },
          "width_m": {
            "title": "Width M",
            "type": "number"
          }
        },
        "required": [
          "length_m",
          "width_m",
          "coordinate_contract"
        ],
        "title": "PitchResponse",
        "type": "object"
      },
      "ReplayEntityResponse": {
        "additionalProperties": false,
        "properties": {
          "entity_id": {
            "title": "Entity Id",
            "type": "string"
          },
          "entity_type": {
            "title": "Entity Type",
            "type": "string"
          },
          "team_id": {
            "title": "Team Id",
            "type": "string"
          },
          "team_role": {
            "title": "Team Role",
            "type": "string"
          },
          "x_m": {
            "title": "X M",
            "type": "number"
          },
          "y_m": {
            "title": "Y M",
            "type": "number"
          }
        },
        "required": [
          "team_id",
          "team_role",
          "entity_id",
          "entity_type",
          "x_m",
          "y_m"
        ],
        "title": "ReplayEntityResponse",
        "type": "object"
      },
      "ReplayFrameResponse": {
        "additionalProperties": false,
        "properties": {
          "entities": {
            "items": {
              "$ref": "#/$defs/ReplayEntityResponse"
            },
            "title": "Entities",
            "type": "array"
          },
          "frame_id": {
            "title": "Frame Id",
            "type": "integer"
          },
          "timestamp_utc": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Timestamp Utc"
          }
        },
        "required": [
          "frame_id",
          "entities"
        ],
        "title": "ReplayFrameResponse",
        "type": "object"
      },
      "ReplayPayloadResponse": {
        "additionalProperties": false,
        "properties": {
          "anchor_frame_id": {
            "title": "Anchor Frame Id",
            "type": "integer"
          },
          "canonical_sources": {
            "additionalProperties": {
              "type": "string"
            },
            "title": "Canonical Sources",
            "type": "object"
          },
          "end_frame_id": {
            "title": "End Frame Id",
            "type": "integer"
          },
          "frame_rate_hz": {
            "title": "Frame Rate Hz",
            "type": "number"
          },
          "frames": {
            "items": {
              "$ref": "#/$defs/ReplayFrameResponse"
            },
            "title": "Frames",
            "type": "array"
          },
          "generated_at": {
            "title": "Generated At",
            "type": "string"
          },
          "match_id": {
            "title": "Match Id",
            "type": "string"
          },
          "overlays": {
            "additionalProperties": true,
            "title": "Overlays",
            "type": "object"
          },
          "period": {
            "title": "Period",
            "type": "string"
          },
          "pitch": {
            "$ref": "#/$defs/PitchResponse"
          },
          "replay_window_id": {
            "title": "Replay Window Id",
            "type": "string"
          },
          "schema_version": {
            "title": "Schema Version",
            "type": "string"
          },
          "source_id": {
            "title": "Source Id",
            "type": "string"
          },
          "source_kind": {
            "enum": [
              "result",
              "target",
              "chain_record",
              "certified_table_partition"
            ],
            "title": "Source Kind",
            "type": "string"
          },
          "start_frame_id": {
            "title": "Start Frame Id",
            "type": "integer"
          }
        },
        "required": [
          "schema_version",
          "replay_window_id",
          "source_kind",
          "source_id",
          "match_id",
          "period",
          "frame_rate_hz",
          "start_frame_id",
          "end_frame_id",
          "anchor_frame_id",
          "generated_at",
          "canonical_sources",
          "pitch",
          "frames"
        ],
        "title": "ReplayPayloadResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "inspection": {
        "additionalProperties": true,
        "title": "Inspection",
        "type": "object"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "replay": {
        "$ref": "#/$defs/ReplayPayloadResponse"
      },
      "replay_window": {
        "additionalProperties": true,
        "title": "Replay Window",
        "type": "object"
      }
    },
    "required": [
      "ok",
      "inspection",
      "replay_window",
      "replay"
    ],
    "title": "InspectResultResponseEnvelope",
    "type": "object"
  },
  "InspectTimestampResponse": {
    "$defs": {
      "PitchResponse": {
        "additionalProperties": false,
        "properties": {
          "coordinate_contract": {
            "title": "Coordinate Contract",
            "type": "string"
          },
          "length_m": {
            "title": "Length M",
            "type": "number"
          },
          "width_m": {
            "title": "Width M",
            "type": "number"
          }
        },
        "required": [
          "length_m",
          "width_m",
          "coordinate_contract"
        ],
        "title": "PitchResponse",
        "type": "object"
      },
      "ReplayEntityResponse": {
        "additionalProperties": false,
        "properties": {
          "entity_id": {
            "title": "Entity Id",
            "type": "string"
          },
          "entity_type": {
            "title": "Entity Type",
            "type": "string"
          },
          "team_id": {
            "title": "Team Id",
            "type": "string"
          },
          "team_role": {
            "title": "Team Role",
            "type": "string"
          },
          "x_m": {
            "title": "X M",
            "type": "number"
          },
          "y_m": {
            "title": "Y M",
            "type": "number"
          }
        },
        "required": [
          "team_id",
          "team_role",
          "entity_id",
          "entity_type",
          "x_m",
          "y_m"
        ],
        "title": "ReplayEntityResponse",
        "type": "object"
      },
      "ReplayFrameResponse": {
        "additionalProperties": false,
        "properties": {
          "entities": {
            "items": {
              "$ref": "#/$defs/ReplayEntityResponse"
            },
            "title": "Entities",
            "type": "array"
          },
          "frame_id": {
            "title": "Frame Id",
            "type": "integer"
          },
          "timestamp_utc": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Timestamp Utc"
          }
        },
        "required": [
          "frame_id",
          "entities"
        ],
        "title": "ReplayFrameResponse",
        "type": "object"
      },
      "ReplayPayloadResponse": {
        "additionalProperties": false,
        "properties": {
          "anchor_frame_id": {
            "title": "Anchor Frame Id",
            "type": "integer"
          },
          "canonical_sources": {
            "additionalProperties": {
              "type": "string"
            },
            "title": "Canonical Sources",
            "type": "object"
          },
          "end_frame_id": {
            "title": "End Frame Id",
            "type": "integer"
          },
          "frame_rate_hz": {
            "title": "Frame Rate Hz",
            "type": "number"
          },
          "frames": {
            "items": {
              "$ref": "#/$defs/ReplayFrameResponse"
            },
            "title": "Frames",
            "type": "array"
          },
          "generated_at": {
            "title": "Generated At",
            "type": "string"
          },
          "match_id": {
            "title": "Match Id",
            "type": "string"
          },
          "overlays": {
            "additionalProperties": true,
            "title": "Overlays",
            "type": "object"
          },
          "period": {
            "title": "Period",
            "type": "string"
          },
          "pitch": {
            "$ref": "#/$defs/PitchResponse"
          },
          "replay_window_id": {
            "title": "Replay Window Id",
            "type": "string"
          },
          "schema_version": {
            "title": "Schema Version",
            "type": "string"
          },
          "source_id": {
            "title": "Source Id",
            "type": "string"
          },
          "source_kind": {
            "enum": [
              "result",
              "target",
              "chain_record",
              "certified_table_partition"
            ],
            "title": "Source Kind",
            "type": "string"
          },
          "start_frame_id": {
            "title": "Start Frame Id",
            "type": "integer"
          }
        },
        "required": [
          "schema_version",
          "replay_window_id",
          "source_kind",
          "source_id",
          "match_id",
          "period",
          "frame_rate_hz",
          "start_frame_id",
          "end_frame_id",
          "anchor_frame_id",
          "generated_at",
          "canonical_sources",
          "pitch",
          "frames"
        ],
        "title": "ReplayPayloadResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "inspection": {
        "additionalProperties": true,
        "title": "Inspection",
        "type": "object"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "replay": {
        "$ref": "#/$defs/ReplayPayloadResponse"
      },
      "replay_window": {
        "additionalProperties": true,
        "title": "Replay Window",
        "type": "object"
      }
    },
    "required": [
      "ok",
      "inspection",
      "replay_window",
      "replay"
    ],
    "title": "InspectTimestampResponseEnvelope",
    "type": "object"
  },
  "InterpretResponse": {
    "$defs": {
      "CapabilityGapResponse": {
        "additionalProperties": false,
        "properties": {
          "concept": {
            "title": "Concept",
            "type": "string"
          },
          "reason": {
            "title": "Reason",
            "type": "string"
          }
        },
        "required": [
          "concept",
          "reason"
        ],
        "title": "CapabilityGapResponse",
        "type": "object"
      },
      "RecipeCardResponse": {
        "additionalProperties": false,
        "properties": {
          "allowed_claims": {
            "items": {
              "type": "string"
            },
            "title": "Allowed Claims",
            "type": "array"
          },
          "description": {
            "title": "Description",
            "type": "string"
          },
          "disallowed_claims": {
            "items": {
              "type": "string"
            },
            "title": "Disallowed Claims",
            "type": "array"
          },
          "display_name": {
            "title": "Display Name",
            "type": "string"
          },
          "limitations": {
            "items": {
              "type": "string"
            },
            "title": "Limitations",
            "type": "array"
          },
          "output_classifications": {
            "items": {
              "type": "string"
            },
            "title": "Output Classifications",
            "type": "array"
          },
          "recipe_id": {
            "title": "Recipe Id",
            "type": "string"
          },
          "recipe_version": {
            "title": "Recipe Version",
            "type": "string"
          },
          "state": {
            "enum": [
              "APPROVED",
              "EXPERIMENTAL",
              "USER_SAVED",
              "DEPRECATED"
            ],
            "title": "State",
            "type": "string"
          }
        },
        "required": [
          "recipe_id",
          "recipe_version",
          "state",
          "display_name",
          "description",
          "allowed_claims",
          "disallowed_claims",
          "limitations",
          "output_classifications"
        ],
        "title": "RecipeCardResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "agent_session_id": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Agent Session Id"
      },
      "bound_plan_hash": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Bound Plan Hash"
      },
      "bound_plan_id": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Bound Plan Id"
      },
      "capability_gaps": {
        "anyOf": [
          {
            "items": {
              "$ref": "#/$defs/CapabilityGapResponse"
            },
            "type": "array"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Capability Gaps"
      },
      "clarification_codes": {
        "anyOf": [
          {
            "items": {
              "type": "string"
            },
            "type": "array"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Clarification Codes"
      },
      "clarification_questions": {
        "anyOf": [
          {
            "items": {
              "type": "string"
            },
            "type": "array"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Clarification Questions"
      },
      "draft_plan_hash": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Draft Plan Hash"
      },
      "draft_plan_id": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Draft Plan Id"
      },
      "fallback_reason": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Fallback Reason"
      },
      "manual_available": {
        "anyOf": [
          {
            "type": "boolean"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Manual Available"
      },
      "message": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Message"
      },
      "model_session_id": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Model Session Id"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "plan_document": {
        "anyOf": [
          {
            "additionalProperties": true,
            "type": "object"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Plan Document"
      },
      "plan_hash": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Plan Hash"
      },
      "provenance_source": {
        "enum": [
          "REVIEWED_RECIPE",
          "MANUAL_PRESET",
          "HERMES_RECIPE_SELECTION",
          "HERMES_NOVEL_COMPOSITION",
          "HERMES_EXPERIMENTAL_UNVERIFIED",
          "DETERMINISTIC_REPAIR",
          "CAPABILITY_GAP",
          "MODEL_UNAVAILABLE"
        ],
        "title": "Provenance Source",
        "type": "string"
      },
      "query": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Query"
      },
      "recipe": {
        "anyOf": [
          {
            "$ref": "#/$defs/RecipeCardResponse"
          },
          {
            "type": "null"
          }
        ],
        "default": null
      },
      "recipe_id": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Recipe Id"
      },
      "repair_applied": {
        "default": false,
        "title": "Repair Applied",
        "type": "boolean"
      },
      "source": {
        "anyOf": [
          {
            "type": "string"
          },
          {
            "type": "null"
          }
        ],
        "default": null,
        "title": "Source"
      },
      "status": {
        "enum": [
          "PLAN_INTERPRETED",
          "CLARIFICATION_REQUIRED",
          "CAPABILITY_GAP",
          "MODEL_UNAVAILABLE"
        ],
        "title": "Status",
        "type": "string"
      }
    },
    "required": [
      "ok",
      "status",
      "provenance_source"
    ],
    "title": "InterpretResponse",
    "type": "object"
  },
  "MatchLibraryResponse": {
    "$defs": {
      "MatchSummaryResponse": {
        "additionalProperties": false,
        "properties": {
          "away_team": {
            "title": "Away Team",
            "type": "string"
          },
          "away_team_brand": {
            "$ref": "#/$defs/TeamBrandResponse"
          },
          "home_team": {
            "title": "Home Team",
            "type": "string"
          },
          "home_team_brand": {
            "$ref": "#/$defs/TeamBrandResponse"
          },
          "kickoff_time_utc": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Kickoff Time Utc"
          },
          "match_day": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Match Day"
          },
          "match_id": {
            "title": "Match Id",
            "type": "string"
          },
          "match_title": {
            "title": "Match Title",
            "type": "string"
          },
          "result": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Result"
          }
        },
        "required": [
          "match_id",
          "match_title",
          "home_team",
          "away_team",
          "home_team_brand",
          "away_team_brand"
        ],
        "title": "MatchSummaryResponse",
        "type": "object"
      },
      "TeamBrandResponse": {
        "additionalProperties": false,
        "properties": {
          "abbreviation": {
            "title": "Abbreviation",
            "type": "string"
          },
          "logo_source": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Logo Source"
          },
          "logo_url": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Logo Url"
          },
          "primary_color": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Primary Color"
          },
          "secondary_color": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Secondary Color"
          },
          "short_name": {
            "title": "Short Name",
            "type": "string"
          },
          "team_id": {
            "anyOf": [
              {
                "type": "string"
              },
              {
                "type": "null"
              }
            ],
            "default": null,
            "title": "Team Id"
          },
          "team_name": {
            "title": "Team Name",
            "type": "string"
          }
        },
        "required": [
          "team_name",
          "short_name",
          "abbreviation"
        ],
        "title": "TeamBrandResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "default_match_ids": {
        "items": {
          "type": "string"
        },
        "title": "Default Match Ids",
        "type": "array"
      },
      "matches": {
        "items": {
          "$ref": "#/$defs/MatchSummaryResponse"
        },
        "title": "Matches",
        "type": "array"
      },
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "perspective_team": {
        "title": "Perspective Team",
        "type": "string"
      },
      "perspective_team_brand": {
        "$ref": "#/$defs/TeamBrandResponse"
      }
    },
    "required": [
      "ok",
      "perspective_team",
      "perspective_team_brand",
      "default_match_ids",
      "matches"
    ],
    "title": "MatchLibraryResponse",
    "type": "object"
  },
  "PlanResponse": {
    "$defs": {
      "RecipeCardResponse": {
        "additionalProperties": false,
        "properties": {
          "allowed_claims": {
            "items": {
              "type": "string"
            },
            "title": "Allowed Claims",
            "type": "array"
          },
          "description": {
            "title": "Description",
            "type": "string"
          },
          "disallowed_claims": {
            "items": {
              "type": "string"
            },
            "title": "Disallowed Claims",
            "type": "array"
          },
          "display_name": {
            "title": "Display Name",
            "type": "string"
          },
          "limitations": {
            "items": {
              "type": "string"
            },
            "title": "Limitations",
            "type": "array"
          },
          "output_classifications": {
            "items": {
              "type": "string"
            },
            "title": "Output Classifications",
            "type": "array"
          },
          "recipe_id": {
            "title": "Recipe Id",
            "type": "string"
          },
          "recipe_version": {
            "title": "Recipe Version",
            "type": "string"
          },
          "state": {
            "enum": [
              "APPROVED",
              "EXPERIMENTAL",
              "USER_SAVED",
              "DEPRECATED"
            ],
            "title": "State",
            "type": "string"
          }
        },
        "required": [
          "recipe_id",
          "recipe_version",
          "state",
          "display_name",
          "description",
          "allowed_claims",
          "disallowed_claims",
          "limitations",
          "output_classifications"
        ],
        "title": "RecipeCardResponse",
        "type": "object"
      }
    },
    "additionalProperties": false,
    "properties": {
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "plan_document": {
        "additionalProperties": true,
        "title": "Plan Document",
        "type": "object"
      },
      "plan_hash": {
        "title": "Plan Hash",
        "type": "string"
      },
      "recipe": {
        "$ref": "#/$defs/RecipeCardResponse"
      }
    },
    "required": [
      "ok",
      "recipe",
      "plan_document",
      "plan_hash"
    ],
    "title": "PlanResponse",
    "type": "object"
  },
  "SubmitValidateResponse": {
    "additionalProperties": false,
    "properties": {
      "ok": {
        "const": true,
        "title": "Ok",
        "type": "boolean"
      },
      "submit": {
        "additionalProperties": true,
        "title": "Submit",
        "type": "object"
      },
      "validation": {
        "additionalProperties": true,
        "title": "Validation",
        "type": "object"
      }
    },
    "required": [
      "ok",
      "submit",
      "validation"
    ],
    "title": "SubmitValidateResponseEnvelope",
    "type": "object"
  }
} as const;

export type BootstrapResponse = FromSchema<typeof apiSchemas["BootstrapResponse"]>;
export type CoachInterpretResponse = FromSchema<typeof apiSchemas["CoachInterpretResponse"]>;
export type ConfirmationResponse = FromSchema<typeof apiSchemas["ConfirmationResponse"]>;
export type ErrorResponse = FromSchema<typeof apiSchemas["ErrorResponse"]>;
export type ExecutionProgressResponse = FromSchema<typeof apiSchemas["ExecutionProgressResponse"]>;
export type ExecutionResponse = FromSchema<typeof apiSchemas["ExecutionResponse"]>;
export type FilmRoomAskResponse = FromSchema<typeof apiSchemas["FilmRoomAskResponse"]>;
export type FilmRoomBootstrapResponse = FromSchema<typeof apiSchemas["FilmRoomBootstrapResponse"]>;
export type FilmRoomReplayFrameResponse = FromSchema<typeof apiSchemas["FilmRoomReplayFrameResponse"]>;
export type FilmRoomReplayWindowResponse = FromSchema<typeof apiSchemas["FilmRoomReplayWindowResponse"]>;
export type HealthResponse = FromSchema<typeof apiSchemas["HealthResponse"]>;
export type InspectResultResponse = FromSchema<typeof apiSchemas["InspectResultResponse"]>;
export type InspectTimestampResponse = FromSchema<typeof apiSchemas["InspectTimestampResponse"]>;
export type InterpretResponse = FromSchema<typeof apiSchemas["InterpretResponse"]>;
export type MatchLibraryResponse = FromSchema<typeof apiSchemas["MatchLibraryResponse"]>;
export type PlanResponse = FromSchema<typeof apiSchemas["PlanResponse"]>;
export type SubmitValidateResponse = FromSchema<typeof apiSchemas["SubmitValidateResponse"]>;
