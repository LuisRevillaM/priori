...
function check(status: CheckStatus, id: string, message: string, evidence?: string): Check {
...
function bundleRecords(manifest: JsonObject, repoRoot: string): EvidenceBundleRecord[] {
...
function validateReplay(record: EvidenceBundleRecord, bundle: JsonObject, replay: JsonObject): Check[] {
  const checks: Check[] = [];
  ...
  checks.push(
    check(
      bundle.result_id === record.resultId && replay.result_id === record.resultId ? "pass" : "fail",
      `replay.${record.resultId}.result_id`,
      "Bundle and replay reference the manifest result id.",
      record.bundleJson
    )
  );
  checks.push(
    check(
      typeof bundleQuery.query_hash === "string" && bundleQuery.query_hash.length === 64 ? "pass" : "fail",
      `replay.${record.resultId}.query_hash`,
      "Bundle carries a frozen query hash.",
      record.bundleJson
    )
  );
  checks.push(
    check(
      frames.length > 0 && entityCount > 0 ? "pass" : "fail",
      `replay.${record.resultId}.nonempty`,
      "Replay contains frames and entity observations.",
      record.replayJson
    )
  );
  checks.push(
    check(
      ballFrameCount === frames.length ? "pass" : "fail",
      `replay.${record.resultId}.ball_present`,
      "Every replay frame includes a ball observation.",
      record.replayJson
    )
  );
  checks.push(
    check(
      invalidCoordinateCount === 0 ? "pass" : "fail",
      `replay.${record.resultId}.coordinate_bounds`,
      "Replay coordinates stay within pitch plus tolerance.",
      record.replayJson
    )
  );
  return checks;
...
function main(): number {
  ...
  checks.push(
    check(
      ...
      "manifest.status",
      "Proof manifest reports pass.",
      manifestPath
    )
  );
  checks.push(
    check(
      records.length >= 8 && records.length === asNumber(manifest.selected_result_count, "manifest.selected_result_count")
        ? "pass"
        : "fail",
      "manifest.selected_result_count",
      "Manifest records the selected evidence-bundle count.",
      manifestPath
    )
  );
  ...
  const status: CheckStatus = summary.fail === 0 ? "pass" : "fail";
